...
func GetPrincipal(ctx context.Context, organizationID, projectID string) (string, string, error) {
	p, err := FromContext(ctx)
	if err != nil {
		if !goerrors.Is(err, coreerrors.ErrInvalidContext) {
			return "", "", errors.OAuth2ServerError("unable to get principal").WithError(err)
		}

		return organizationID, projectID, nil
	}

	return p.OrganizationID, p.ProjectID, nil
}

...
// errorHandler wraps up error redirects.
...
func newErrorHandler(w http.ResponseWriter, r *http.Request, redirectURI, state string) *errorHandler {
	return &errorHandler{
		w:           w,
		r:           r,
...
// code in the query.
func (e *errorHandler) raise(kind Error, description string) {
	values := &url.Values{}
...
func authorizationValidateRedirecting(errorHandler *errorHandler, query url.Values) bool {
	var kind Error
	...
	case !slices.Contains(allowedResponseTypes, query.Get("response_type")):
		kind = ErrorUnsupportedResponseType
		description = "response_type must be one of " + strings.Join(allowedResponseTypes, ", ")
	case !slices.Contains(allowedResponseModes, query.Get("response_mode")):
		kind = ErrorUnsupportedResponseType
		description = "response_mode must be one of " + strings.Join(allowedResponseModes, ", ")
	case !slices.Contains(allowedCodeChallengeMethods, string(getCodeChallengeMethod(query))):
		kind = ErrorInvalidRequest
		description = "code_challenge_method must be one of " + strings.Join(allowedCodeChallengeMethods, ", ")
	default:
		return true
	}

	errorHandler.raise(kind, description)
...
func (a *Authenticator) Authorization(w http.ResponseWriter, r *http.Request) {
	...
	errorHandler := newErrorHandler(w, r, client.Spec.RedirectURI, query.Get("state"))
	...
	if !authorizationValidateRedirecting(errorHandler, query) {
		return
	}
	...
	if query.Get("prompt") == "none" {
		errorHandler.raise(ErrorInteractionRequired, "login required but no prompt requested")
		return
	}
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "failed to encode request state")
		return
	}

	supportedTypes, err := a.getProviderTypes(r.Context())
	if err != nil {
		errorHandler.raise(ErrorServerError, "failed to get oauth2 providers")
		return
	}
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "failed to render login template")
		return
	}
...
func (a *Authenticator) Login(w http.ResponseWriter, r *http.Request) {
	...
	errorHandler := newErrorHandler(w, r, query.Get("redirect_uri"), query.Get("state"))
	...
	if providerType := r.Form.Get("provider"); providerType != "" {
		...
		if err != nil {
			errorHandler.raise(ErrorServerError, err.Error())
			return
		}

		a.providerAuthenticationRequest(w, r, errorHandler, provider, query, "")
	...
	if email == "" {
		errorHandler.raise(ErrorServerError, "email query parameter not specified")
		return
	}
	...
	a.providerAuthenticationRequest(w, r, errorHandler, provider, query, email)
}

// providerAuthenticationRequest kicks off the authorization flow with the backend
// provider.
func (a *Authenticator) providerAuthenticationRequest(w http.ResponseWriter, r *http.Request, errorHandler *errorHandler, provider *unikornv1.OAuth2Provider, query url.Values, email string) {
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "unable to create oidc nonce: "+err.Error())
		return
	}
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "unable to create oauth2 code verifier: "+err.Error())
		return
	}
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "failed to encode oidc state: "+err.Error())
		return
	}
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "unable to create oauth2 config: "+err.Error())
		return
	}
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "unable to create oauth2 redirect: "+err.Error())
		return
	}
...
func (a *Authenticator) Callback(w http.ResponseWriter, r *http.Request) {
	...
	errorHandler := newErrorHandler(w, r, clientQuery.Get("redirect_uri"), clientQuery.Get("state"))

	if query.Has("error") {
		errorHandler.raise(Error(query.Get("error")), query.Get("error_description"))
		return
	}

	if !query.Has("code") {
		errorHandler.raise(ErrorServerError, "oidc callback does not contain an authorization code")
		return
	}

	provider, err := a.lookupProviderByID(r.Context(), state.OAuth2Provider, nil)
	if err != nil {
		errorHandler.raise(ErrorServerError, "failed to get oauth2 provider")
		return
	}
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "code exchange failed: "+err.Error())
		return
	}
	...
	if err != nil && !a.options.AuthenticateUnknownUsers {
		errorHandler.raise(ErrorAccessDenied, "user does not exist or is inactive")
		return
	}
	...
	if err != nil {
		errorHandler.raise(ErrorServerError, "failed to encode authorization code: "+err.Error())
		return
	}
	...
	http.Redirect(w, r, clientQuery.Get("redirect_uri")+"?"+q.Encode(), http.StatusFound)
}
